document.addEventListener("DOMContentLoaded", () => {
    ...
    Promise.all([loadPostcodes(), loadValidationMessages()])
      .then(() => {
        ...
        const postcodeInputWrapper = document.querySelector(".postcode-input-wrapper"); // Postcode Input Wrapper
  
        // Ensure elements are found
        if (!postcodeInput || !postcodeClear || !stubCheckButton || !typeformButtonContainer || !responseElement || !postcodeInputWrapper) {
        ...
        postcodeInput.addEventListener("input", (event) => {
          ...
          const isCompleteMatch = completePatterns.some((pattern) => pattern.test(input));
  
          // Check if input is a partial match for any pattern
          const isPartialMatch = partialPatterns.some((pattern) => pattern.test(input));
